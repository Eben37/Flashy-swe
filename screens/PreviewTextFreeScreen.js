...
import { StyleSheet, View, Text, ScrollView, TouchableOpacity, Modal, TextInput, ActivityIndicator, SafeAreaView, KeyboardAvoidingView, Platform } from 'react-native';
import axios from 'axios';
import { useNavigation } from '@react-navigation/native';
import Icon from 'react-native-vector-icons/Ionicons';

const CustomCheckbox = ({ isChecked, onToggle, disabled }) => (
  <TouchableOpacity onPress={disabled ? null : onToggle} style={[styles.checkboxContainer, disabled && styles.checkboxDisabled]}>
    <View style={[styles.checkbox, isChecked && styles.checkboxChecked, disabled && styles.checkboxDisabled]}>
      {isChecked && <Text style={styles.checkMark}>✓</Text>}
    </View>
    <Text style={[styles.label, disabled && styles.labelDisabled]}>Select this page</Text>
  </TouchableOpacity>
);

const PreviewTextFreeScreen = ({ route }) => {
  const { pages, fileType } = route.params || {};
  ...
  const handleFormSubmit = async () => {
    if (flashcardNumber < 1 || flashcardNumber > 5) {
      setErrorMessage('The maximum number of flashcards is 5. Please enter a number within the limit.');
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>Preview Document Pages</Text>
      <View style={styles.previewBox}>
        {pages.map((page, index) => (
          <View key={index} style={[styles.pagePreview, index >= 5 && styles.grayedOut]}>
            <View>
            ...
            <View>
              <CustomCheckbox
                ...
                disabled={index >= 5} // Disable checkbox if page is grayed out
              />
            </View>
          </View>
        ))}
...
export default PreviewTextFreeScreen;
